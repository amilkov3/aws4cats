...
import aws4cats.{AccountId, ExecutorServiceWrapper, sqs}
...
sealed abstract class AsyncSQSClient[F[_]](client: SqsAsyncClient)(
  ...
  override def getQueueAttributes(queue: Uri,
                                  attributes: List[QueueAttributeName])
    : F[Map[QueueAttributeName, String]] =
    A.async { cb =>
  ...
  override def getQueueUrl(queueName: QueueName,
